...
def scan_files(directory, tree, label):
    """
    ...
    label.config(text=f"Scan Speed: {speed:.2f} bit/s")

def scan_selected_directory(tree, label):
    """
    选择目录并扫描。
    """
    directory = filedialog.askdirectory()
    if directory:
        scan_files(directory, tree, label)
...
# 创建速度显示标签
speed_label = tk.Label(app, text="Scan Speed: 0 bit/s")
speed_label.pack(pady=10)

# 创建按钮
scan_button = tk.Button(app, text="Scan Directory", command=lambda: scan_selected_directory(tree, speed_label))
